...
If `configureUserFromGit` is enabled (default), it will run on `postStartCommand` (container start) and read `git config --global user.name` / `user.email`, then set `jj config set --user user.name` / `user.email`. This is skipped if `${HOME}/.config/jj/` already exists.
